...
public abstract class TwoWayAbsListView extends TwoWayAdapterView<ListAdapter> implements ViewTreeObserver.OnTouchModeChangeListener {
    ...
    /**
     * Indicates the touch has been recognized as a tap and we are now waiting to
     * see if the touch is a longpress
     */
    ...
    /**
     * Indicates the vieif (ViewDebug.TRACE_RECYCLER) { ViewDebug.trace(child,
     * ViewDebug.RecyclerTraceType.MOVE_TO_SCRAP_HEAP, firstPosition + i, -1); }w
     * is in the process of being flung
     */
    ...
    /**
     * Layout to sync as a result of a data change. Restore mSyncPosition to have
     * its top at mSpecificTop
     */
    ...
    /**
     * Indicates whether the list selector should be drawn on top of the children
     * or behind
     */
    ...
    public interface OnScrollListener {
        ...
        /**
         * The view is not scrolling. Note navigating the list using the trackball
         * counts as being in the idle state since these transitions are not
         * animated.
         */
        ...
        /**
         * The user had previously been scrolling using touch and had performed a
         * fling. The animation is now coasting to a stop
         */
     ...
     * to implement {@link SectionIndexer} if it wishes to display alphabet
    ...
    */
    ...
    /**
     * When stack from bottom is set to true, the list fills its content starting
     * from the bottom of the view.
     *
     * @param stackFromBottom true to pin the view's content to the bottom edge, false to pin
     *                        the view's content to the top edge
     */
    ...
    public Parcelable onSaveInstanceState() {
      /*
       * This doesn't really make sense as the place to dismiss the popups, but
       * there don't seem to be any other useful hooks that happen early enough
       * to keep from getting complaints about having leaked the window.
       */
        // dismissPopup();
    ...
    public void onRestoreInstanceState(Parcelable state) {
        ...
        if (ss.selectedId >= 0) {
            ...
            // Do this before setting mNeedSync since setNextSelectedPosition looks
            // at mNeedSync
    ...
    */
     ...
     *                 the returned view was taken from the scrap heap, false if
     *                 otherwise.
    ...
    protected void dispatchDraw(Canvas canvas) {
       ...
       * getBottom() - getTop() - getPaddingBottom()); mGroupFlags &=
     ...
     * This methods knows the view, position and ID of the item that received the
     * long press.
     *
    ...
    */
    ...
    /**
     * Fills the gap left open by a touch-scroll. During a touch scroll, children
     * that remain on screen are shifted and the other ones are discarded. The
     * role of this method is to fill the gap thus created by performing a
    ...
    /**
     * Attempt to bring the selection back if the user is switching from touch to
     * trackball mode
     *
            ...
            if (mNeedSync) {
                ...
                switch (mSyncMode) {
                    case SYNC_SELECTED_POSITION:
                        if (isInTouchMode()) {
                            // We saved our state when not in touch mode. (We know this
                            // because
                            // mSyncMode is SYNC_SELECTED_POSITION.) Now we are trying to
                            // restore in touch mode. Just leave mSyncPosition as it is
                            // (possibly
                            ...
                            if (newPos >= 0) {
                                ...
                                if (selectablePos == newPos) {
                                    ...
                                    if (mSyncSize == size) {
                                        // If we are at the same height as when we saved
                                        // state, try
     ...
     * or {@link #TRANSCRIPT_MODE_ALWAYS_SCROLL}
     */
     ...
     * TwoWayAbsListView into the supplied List. This includes views displayed on
     * the screen as well as views stored in TwoWayAbsListView's internal view
     * recycler.
     *
    ...
    /**
     * TwoWayAbsListView extends LayoutParams to provide a place to hold the view
     * type.
     */
    public static class LayoutParams extends RelativeLayout.LayoutParams {
        ...
        /**
         * When an TwoWayAbsListView is measured with an AT_MOST measure spec, it
         * needs to obtain children views to measure itself. When doing so, the
         * children are not attached to the window, but put in the recycler which
         * assumes they've been attached before. Setting this flag will force the
         * reused view to be attached to the window rather than just attached to
         * the parent.
         */
        @ViewDebug.ExportedProperty
        boolean forceAdd;
     ...
     * those views which were onscreen at the start of a layout. By construction,
     * they are displaying current information. At the end of layout, all views
     * in ActiveViews are demoted to ScrapViews. ScrapViews are old views that
     * could potentially be used by the adapter to avoid allocating views
     * unnecessarily.
     *
     * @see com.jess.ui.TwoWayAbsListView#setRecyclerListener(com.jess.ui.TwoWayAbsListView.RecyclerListener)
     * @see com.jess.ui.TwoWayAbsListView.RecyclerListener
    ...
    class RecycleBin {
         ...
         * populated at the start of layout, and at the end of layout all view in
         * mActiveViews are moved to mScrapViews. Views in mActiveViews represent
         * a contiguous range of Views, with position of the first view store in
        ...
        void fillActiveViews(int childCount, int firstActivePosition) {
            ...
            for (int i = 0; i < childCount; i++) {
                ...
                if (lp != null && lp.viewType != ITEM_VIEW_TYPE_HEADER_OR_FOOTER) {
                    // Note: We do place TwoWayAdapterView.ITEM_VIEW_TYPE_IGNORE in
                    // active views.
        ...
        /**
         * Get the view corresponding to the specified position. The view will be
         * removed from mActiveViews if it is found.
         *
    ...
    abstract class TouchHandler {
        ...
        public boolean startScrollIfNeeded(int delta) {
            ...
            if (distance > mTouchSlop) {
                ...
                // Handler should not be null unless the TwoWayAbsListView is not
                // attached to a
         ...
         * state change is fired only if the specified state is different from the
         * previously known state.
         *
         * @param newState The new scroll state.
         */
        ...
        /**
         * Smoothly scroll to the specified adapter position. The view will scroll
         * such that the indicated position is displayed, but it will stop early
         * if scrolling further would scroll boundPosition out of view.
         *
         * @param position      Scroll to this adapter position.
         * @param boundPosition Do not scroll if it would move this adapter position out of
         *                      view.
         ...
         *
         * @param delta            Amount to offset mMotionView. This is the accumulated delta
         *                         since the motion began. Positive numbers mean the user's
         *                         finger is moving down or right on the screen.
    ...
    class VerticalTouchHandler extends TouchHandler {
                        ...
                        case TOUCH_MODE_SCROLL:
                            ...
                            if (y != mLastY) {
                                ...
                                if (atEdge && getChildCount() > 0) {
                                    ...
                                    // into
                                    // content immediately rather than needing to scroll back
                                    // to the
        ...
        public boolean onInterceptTouchEvent(MotionEvent ev) {
         ...
         /*
          * if (mFastScroller != null) { boolean intercepted =
          * mFastScroller.onInterceptTouchEvent(ev); if (intercepted) { return
          * true; } }
          */
